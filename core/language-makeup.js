	...
	folderContents.forEach((item) => {
		...
		if (fs.statSync(itemPath).isDirectory()) {
			...
			if (extensions[fileExtension]
				&& pathPieces[pathPieces.length - 4].toLowerCase().startsWith('e')) {
...
function getUnusedLanguages() {
	...
	try {
		...
		console.error(`Error in ${getUnusedLanguages.name} in ${__dirname}:\n`, err);
	}
}
...
function getFolderLanguages(folderPath, langsUsed) {
	...
	folderContents.forEach((item) => {
		...
		if (fs.statSync(itemPath).isDirectory()) {
			...
			if (extensions[fileExtension]
				&& pathPieces[pathPieces.length - 4].toLowerCase().startsWith('e')
				&& !isNaN(parseInt(pathPieces[pathPieces.length - 4][1]))
				&& !langsUsed.includes(extensions[fileExtension])) {
